...
class S3toMongoOperator(BaseOperator):
    ...
    template_fields = ("s3_key", "mongo_collection", "mongo_replacement_filter")
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.mongo_method not in ("insert", "replace"):
            raise Exception(
                'Please specify either "insert" or "replace" for Mongo method.'
            )

        if self.upsert is not True and self.upsert is not False:
            raise Exception(
                "Upsert must be specified as a boolean. Please choose True/False."
            )
    ...
    def execute(self, context):
        ...
        data = s3.get_key(
            self.s3_key, bucket_name=self.s3_bucket
    ...
    def insert_records(self, mongo, docs):
        if len(docs) == 1:
            mongo.insert_one(self.mongo_collection, docs[0], mongo_db=self.mongo_db)
        else:
            mongo.insert_many(
                self.mongo_collection, [doc for doc in docs], mongo_db=self.mongo_db
            )
    ...
    def replace_records(self, mongo, docs):
        ...
        for doc in docs:
            ...
            if isinstance(self.mongo_replacement_filter, str):
                mongo_replacement_filter = {
                    self.mongo_replacement_filter: doc.get(
                        self.mongo_replacement_filter, False
                    )
                }
                ...
                for k, v in self.mongo_replacement_filter.items():
                    if k == v:
                        ...
                        mongo_replacement_filter[k] = self.mongo_replacement_filter.get(
                            k, False
                        )
        ...
        if len(operations) > 0:
            ...
            mongo.bulk_write(
                self.mongo_collection, operations, mongo_db=self.mongo_db, ordered=False
            )
